...
def complet_csv(human_csv,delta,triplet_id):
    ...
    df = df[
        ['subject_id','triplet_id', 'language_TGT', 'language_OTH','phone_TGT', 'phone_OTH',
         'context', 'user_ans']]

    # We average for triplet
    gf = df.groupby(['triplet_id', 'language_TGT', 'language_OTH','phone_TGT', 'phone_OTH','context'], as_index=False)
    ...
    gf = df2.groupby(['language_TGT', 'language_OTH','phone_TGT', 'phone_OTH','context'], as_index=False)
    ans = gf.user_ans.mean()
    val_delta = gf.delta_values.mean()
    ans['delta_values'] = val_delta['delta_values']

    # then we average over phone contrast
    gf = ans.groupby(['language_TGT', 'language_OTH','phone_TGT', 'phone_OTH'], as_index=False)
    ans = gf.user_ans.mean()
    val_delta = gf.delta_values.mean()
    ans['delta_values'] = val_delta['delta_values']
    ...
    #print('TOTAL#####', total)
    gf = total.groupby(['language_TGT', 'language_OTH','phone_TGT', 'phone_OTH'], as_index=False)
    ans = gf.user_ans.mean()
    val_delta = gf.delta_values.mean()
    ans['delta_values'] = val_delta['delta_values']
